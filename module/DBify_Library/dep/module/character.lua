    ...
    delete = function(...)
        ...
        return try({
            exec = function(self)
                return self:await(
                    imports.assetify.thread:createPromise(function(resolve, reject)
                        if not characterID or (imports.type(characterID) ~= "number") then return dbify.mysql.util.throwError(reject, syntaxMsg) end
    ...
    setData = function(...)
        ...
        return try({
            exec = function(self)
                return self:await(
                    imports.assetify.thread:createPromise(function(resolve, reject)
                        local characterID, dataColumns = dbify.mysql.util.fetchArg(_, cArgs), dbify.mysql.util.fetchArg(_, cArgs)
                        if not characterID or (imports.type(characterID) ~= "number") or not dataColumns or (imports.type(dataColumns) ~= "table") or (#dataColumns <= 0) then return dbify.mysql.util.throwError(reject, syntaxMsg) end
                        local isExisting = dbify.character.getData(characterID, {dbify.character.connection.key})
                        if not isExisting then return resolve(isExisting, cArgs) end
