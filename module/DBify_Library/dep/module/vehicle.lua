    ...
    delete = function(...)
        ...
        return try({
            exec = function(self)
                return self:await(
                    imports.assetify.thread:createPromise(function(resolve, reject)
                        if not vehicleID or (imports.type(vehicleID) ~= "number") then return dbify.util.throwError(reject, syntaxMsg) end
    ...
    setData = function(...)
        ...
        return try({
            exec = function(self)
                return self:await(
                    imports.assetify.thread:createPromise(function(resolve, reject)
                        local vehicleID, dataColumns = dbify.util.fetchArg(_, cArgs), dbify.util.fetchArg(_, cArgs)
                        if not vehicleID or (imports.type(vehicleID) ~= "number") or not dataColumns or (imports.type(dataColumns) ~= "table") or (#dataColumns <= 0) then return dbify.util.throwError(reject, syntaxMsg) end
                        local isExisting = dbify.vehicle.getData(vehicleID, {dbify.vehicle.connection.key})
                        if not isExisting then return resolve(isExisting, cArgs) end
