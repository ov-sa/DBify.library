                ...
                exec = function(self)
                    return self:await(
                        imports.assetify.thread:createPromise(function(resolve, reject)
                            ...
                            if not dbify.mysql.table.isValid(tableName) then return dbify.util.throwError(reject, imports.string.format(dbify.error["table_non-existent"], tableName)) end
                            ...
                            if keyColumns then
                                ...
                                if not dbify.mysql.column.areValid(tableName, validateColumns) then return dbify.util.throwError(reject, imports.string.format(dbify.error["columns_non-existent"], tableName)) end
                                for i = 1, #keyColumns, 1 do
                                    local j = keyColumns[i]
                ...
                exec = function(self)
                    return self:await(
                        imports.assetify.thread:createPromise(function(resolve, reject)
                            ...
                            if not tableName or (imports.type(tableName) ~= "string") or not columns or (imports.type(columns) ~= "table") or (#columns <= 0) then return dbify.util.throwError(reject, syntaxMsg) end
                            if not dbify.mysql.table.isValid(tableName) then return dbify.util.throwError(reject, imports.string.format(dbify.error["table_non-existent"], tableName)) end
                            if not dbify.mysql.column.areValid(tableName, columns) then return dbify.util.throwError(reject, imports.string.format(dbify.error["columns_non-existent"], tableName)) end
